...
class Books(db.Model):
    ...
    rentals = db.relationship("Rentals", backref="book", lazy="dynamic")

    @property
    def status(self):
        active_rental = self.rentals.filter_by(returned=False).first()
        return "Wypożyczona" if active_rental else "Na stanie"
...
class Rentals(db.Model):
    ...
    def __str__(self):
        ...
        return f"<Rental {self.id} - Book {self.book_id} {status}>"
